...
Lane::Lane(TrafficLight* LIGHTS, float len): 
light(LIGHTS),
laneLength(len), 
totalVehiclesProcessed(0), 
totalEmergencyProcessed(0){}
...
void Lane::update(int timer) 
{
    ...
    if (light->getState() == LightState::GREEN) 
    {
        ...
        {
            if (vehicles.front().isEmergency()) { //Increment emergency count if the departing vehicle is an emergency vehicle 
