...
using namespace std;
    ...
    Stats:: Stats():totalVehiclesProcessed(0), 
    totalWaitTime(0), maxQueueLength(0), tickCount(0){}
    ...
    void Stats:: collect(const Intersection& intersection, int currentTime)
    {
        ...
        totalEmergencyProcessed = intersection.getProcessedEmergencyVehicles(); //Collect total emergency vehicles processed from the intersection
    ...
    void Stats::printSummary() const
    {
        ...
        cout << "Total Emergency Vehicles Processed: " << totalEmergencyProcessed << endl; //Print total emergency vehicles processed
